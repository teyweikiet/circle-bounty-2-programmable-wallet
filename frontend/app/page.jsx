...
import { Accordion, Badge, Tabs, Title, List, ActionIcon, Stack } from '@mantine/core'
import { IconPlus, IconSettings, IconUsers, IconWallet } from '@tabler/icons-react'
...
export default function Page () {
  ...
  const { data: contacts, isLoading: isLoadingContacts } = useListContacts(supabase)

  const items = (data?.wallets || []).map((item) => (
    <Accordion.Item key={item.id} value={item.address}>
  ...
  const handleCreateWallet = async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/wallets`, {
      method: 'POST',
      body: JSON.stringify({
        ...!data?.wallets?.length && { createUserPin: true }
      })
    })
